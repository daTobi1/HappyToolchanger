...
**Fit-Radius selbst ermitteln (Tobi, 2026-09-05):** Knopf „ermitteln" am Feld „Fit-Radius" (`webapp/js/fitradius.js`, Paraboloid-Fit wie in Klipper über das gebinnte Gitter). Sweep 0,75–3,0 mm über das erste Raster jedes Tools; Bezug je Tool ist der Median der Scheitel über alle Radien; je Radius zählt die größte Abweichung davon über alle Tools. Vorschlag = größter Radius, bei dem alle Tools passen, ≥ 9 Punkte haben und die Abweichung ≤ 25 µm und ≤ doppelte Abweichung des besten Radius (+5 µm) bleibt. An Lauf 13: Abweichungen 1,0 → 23, 1,5 → 11, 2,0 → 6, 2,25 → 8, 2,5 → 24, 3,0 → 89 µm; Vorschlag **2,25 mm**, per Knopf als Default ins Feld (Browser) übernehmbar, geht als `FIT_RADIUS` mit jedem Lauf. Das 0,5-mm-Gitter streut den Scheitel je Radius um einige 10 µm, darum kein Nachbarvergleich, sondern das Plateau.

**Z-Offsets aus der Sonde (Tobi, 2026-09-05, separate Option):** Knopf „Z-Offsets aus der Sonde übernehmen + schreiben" im XY-Block (nur Eddy-Verfahren, nur mit `z_compare` im Ergebnis). Rechnung `gcode_z_offset(T) = gcode_z_offset(Referenz, Config) + z_compare(T)`, dieselbe Konvention wie der Z-Switch-Lauf (`z_trigger(n) − z_trigger(ref)`). Im Spaltmodus reproduziert das die Z-Switch-Daten — am Drucker geprüft: Dialog zeigt für T1–T3 exakt die Config-Werte (Δ 0,000), Vorzeichen stimmt. Im Amplitudenmodus warnt der Dialog: Höhe gleicher Amplitude hängt auch vom Signal des Tools ab (T1 8640 gegen T2 11969 Hz bei gleichem Spalt).

**Umstellung auf Amplitudenmodus (Tobi, 2026-09-05):** `Z_MODE` Default jetzt `amplitude` (Modul), `target_amplitude` in den Vorlagen und auf dem 250er von 6.000 auf 12.000 Hz (bei 0,4 mm liefern die Düsen 8.600–16.000 Hz, Lauf 12). Im XY-Block eine Auswahl „Z-Modus" (Amplitude / Spalt), geht als `Z_MODE=` mit jedem Lauf; der Spaltmodus bleibt wählbar. Folgen: der Lauf braucht keine Z-Switch-Daten mehr (kein Bootstrap), die Spalte je Tool sind verschieden (Signal je Spitze verschieden), `z_compare` ist die Höhe gleicher Amplitude — dafür warnt der Z-Knopf. Reproduzierbarkeit im Amplitudenmodus noch nicht mit dem Nachzentrieren gemessen; Lauf 8 (Amplitude, 11 kHz, mit Extrapolation) lag gegen Lauf 9 (Spalt) 0,12–0,25 mm auseinander.

**Modus-Abfrage im Assistenten (Tobi, 2026-09-05):** Nach „Aufsetzen" fragt der Assistent „XY-Messlauf: Z-Modus" mit den Knöpfen „Amplitude" und „Spalt (Z-Switch)"; die Wahl landet in der Auswahl im XY-Block (Browser) und im Kommando. Abbrechen dort bricht den Lauf ab und bietet das Deaktivieren an, weil die Halterung schon steht.

**Automatische Zielamplitude (Tobi, 2026-09-05, nach Lauf 15):** Lauf 15 brach bei T1 ab: am Z-Boden (0,2 mm) nur 11.548 von 12.000 Hz. Zwei Änderungen: (1) `approach_z` misst am Boden, wenn ≥ 85 % des Ziels erreicht sind (`fit.floor_ok`), darunter bleibt es ein Fehler. (2) `TARGET_AMPLITUDE=0` = automatisch: Vorlauf über alle Tools (Wechsel, Grobsuche, bis zum Boden), Ziel = 95 % der schwächsten Bodenamplitude; das schwächste Tool misst knapp über dem Boden, die anderen so tief, wie ihr Signal reicht. Dazu im Amplitudenmodus ein **Z-Boden je Tool** aus den Z-Switch-Daten (`tool_floor`): der feste Boden `holder_top_z + min_gap` gilt für das Referenztool, ein 0,3 mm längeres Tool stünde dort im Kontakt. Die automatische Zielamplitude verlangt deshalb Z-Switch-Daten. Webapp: Haken „Zielamplitude automatisch" (Default an) im XY-Block, geht als `TARGET_AMPLITUDE=0` mit. Kostet je Tool einen Wechsel plus Grobsuche extra (~1 min je Tool).

**Düsentemperatur beim Messlauf (Tobi, 2026-09-06):** `CALIBRATE_XY_OFFSETS TEMP=` gab es schon (jedes Tool wird nach dem Wechsel per M109 geheizt), aber nichts in der Webapp und kein Abschalten. Jetzt: Feld „Düsentemp." im XY-Block (leer/0 = kalt, Browser merkt sich den Wert), geht als `TEMP=` mit jedem Lauf; der Assistent fragt nach dem Z-Modus in einem eigenen Dialog danach. Klipper heizt jedes Tool nach dem Wechsel — auch im Vorlauf der automatischen Zielamplitude, damit Vorlauf und Messung denselben Wärmezustand sehen —, lässt es während des Laufs warm (auch geparkt, kein doppeltes Aufheizen) und schaltet im `finally`, also auch nach einem Abbruch, alle Heizungen des Laufs aus (`_xy_heaters_off`). Warum warm messen: die XY-Offsets gelten bei Drucktemperatur, und der Kopf dehnt sich beim Aufheizen aus; ob das an den Tools messbar ist, zeigt erst ein Vergleichslauf kalt/warm. Zwei Risiken, im Dialog benannt: Filament tropft aus der heißen Düse auf die Spule (0,2–0,4 mm Spalt), und die Platine der Sonde wird von der Düse mit aufgeheizt — die Spulentemperatur steht in jedem Ergebnis, der bidirektionale Sweep fängt den linearen Drift. Noch kein Lauf mit `TEMP>0` gefahren. M109 wartet seit dem 2026-09-05 nur noch bis ins ±4-°C-Band (Totband-Override), die Ausdehnung des Kopfs läuft danach noch nach — wer das ausschließen will, misst mit vorgeheizten Tools zweimal.
