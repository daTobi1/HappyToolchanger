...
class Toolchanger:
    ...
    def _configure_toolhead_for_tool(self, tool):
        ...
        # Activate the corresponding tool_probe (and z_probe if configured)
        tpe = self.printer.lookup_object('tool_probe_endstop', None)
        if tpe:
            if tool and hasattr(tool, 'tool_number'):
                tp = tpe.tool_probes.get(tool.tool_number)
                if tp:
                    tpe.set_active_probe(tp)
            elif not tool:
